...
enum RemoteConfigSource {
  ...
  /// 获取的value值是云端值
  /// The obtained value is the cloud value
  remote,
}
...
class AGCRemoteConfig {
  ...
  /// Pull the latest configuration data from the cloud, the default interval is 12 hours, and the cached data will be returned within 12 hours
  Future<void> fetch({int intervalSeconds = 12}) {
    return _channel.invokeMethod('fetch',
        <String, int>{'intervalSeconds': intervalSeconds}).catchError((e) {
      int code = int.tryParse(e.code) ?? -99;
      int throttleEndTime =
  ...
  /// 返回默认值和云端值合并后的所有值
  Future<Map<String, Object>?> getMergedAll() {
